<form class="flex items-center gap-3" method="post" action="{{ route('add-user', compact('entity')) }}">
    @csrf
    @if (!$show)
        <button wire:click="$set('show', true)" class="bg-transparent border rounded-full border-gray-500 px-3 py-1"
            type="button">
    ...
    @else
        <div class="relative">
            <input type="email" name="email" autofocus autocomplete="off" data-1p-ignore
                class="bg-transparent border rounded-full px-4 py-1 border-gray-500">
